...
class AlbumTrack extends Component {
  ...
  render() {
    ...
    const number =
      this.props.albumTrack !== undefined
        ? this.props.albumTrack.items.length
        : 0;
    return (
      <Container>
        <div>
          ...
          <div style={{ textAlign: "center" }}>
            <h1>{`Bài hát đã thích: ${number}`}</h1>
            <button>{"Phát"}</button>
          </div>
        </div>
        <SongList>
          {this.props.albumTrack !== undefined &&
          this.props.albumTrack.items.length !== 0
            ? this.props.albumTrack.items
                .map((song, index) => {
                  return (
                    <div
                    ...
                    >
                      ...
                      <div>
                        <span>{this.msToTime(song.duration_ms)}</span>
                      </div>
                    </div>
                  );
                })
